...
int main(){
	...
	cout<<"Introduce el vector de los resutlados: "<<endl;
	for(i=0;i<l;i++){
		cout<<"b["<<i<<",0]= ";
		cin>>b[i];
	}
	for(i=0;i>l;i++){
		...
		if(cont==l){
			...
			cout<<"La matriz no es diagoalmente dominate. Proceso finalizado."<<endl;
		}
		system("PAUSE");
		return 0;
	}
void Jacobi(int l, float x[], float a[][10],float b[]){
	...
	line:
		...
		switch(p){
			...
			case 'n':cout<<"Preceso finalizado."<<endl;break;
			return;
		}
} 
